...
namespace Time
{
    ...
    const float getTimeBetween(std::chrono::steady_clock::time_point curr, std::chrono::steady_clock::time_point prev)
    {
        std::chrono::duration<float> elapsed_seconds = curr - prev;
        return elapsed_seconds.count();
    }

    const float getElapsedTime(std::chrono::steady_clock::time_point prevTimePoint)
    {
